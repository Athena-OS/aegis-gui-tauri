<script lang="ts">
  ...
  type ListItem = { name: string, selected?: boolean, size: string, percentage_used:string, display_name: string };
  export let label: string | null = null;
  export let defaultItem: ListItem = { name: "Default Item", size:"", percentage_used:"", display_name:"" };
            ...
            >
              <span
                class="block truncate {selected
                  ? 'font-medium'
                  : 'font-normal'}">{value.display_name} : {value.size}</span
              >
            </li>
          {/each}
        </ul>
